/**
 * Tenant utilities for returns-service
 * Provides helpers for tenant-scoped database operations
 */

import type { Request } from "express";
import type { Prisma } from "@innovabound-ecomm-platform/returns-db";
...
export function returnRequestWhere(
  siteId: string | undefined,
  additionalWhere?: Prisma.ReturnRequestWhereInput,
  ...
  const where: Prisma.ReturnRequestWhereInput = { ...additionalWhere };
  if (siteId) {
    where.siteId = siteId;
  }
  return where;
...
export function returnPolicyWhere(
  siteId: string | undefined,
  additionalWhere?: Prisma.ReturnPolicyWhereInput,
  options: TenantQueryOptions = { strict: true }
): Prisma.ReturnPolicyWhereInput {
  const { strict = true } = options;
  if (strict && !siteId) {
    throw new TenantRequiredError("siteId is required for this query");
  }
  const where: Prisma.ReturnPolicyWhereInput = { ...additionalWhere };
  if (siteId) {
    where.siteId = siteId;
  }
  return where;
